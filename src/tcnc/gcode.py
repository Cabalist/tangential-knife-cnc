...
- Tools: a numbered tool is selected with ``T n M6`` followed by ``G43``,
  ...
  already mounted.
- The header starts with ``Generated by tcnc X``, then ``Created`` when
  the job's ``timestamp`` is on, then the provenance lines when the caller
  supplies them (``tcnc.provenance``). Those identity lines are written
  even when comments are off; every other comment follows
  ``gcode_comments``. ``Created`` is the ``now`` clock, or
  ``SOURCE_DATE_EPOCH`` when that is set and no clock is given.
...
class GCodeWriter:
    ...
    def identity(self, text: str) -> None:
        """Write a comment that identifies the program: written even when comments are off."""
        _reject_control_chars(text, "identity line")
    ...
    def header(self, extra_comments: Iterable[str] = (), provenance: Provenance | None = None) -> None:
        """Program delimiter, identity and provenance lines, comments and modal setup."""
        opts = self.job
        self.raw(PROGRAM_DELIMITER)
        self.identity(f"Generated by tcnc {__version__}")
        if opts.timestamp:
...
def _clock() -> datetime:
    """Now, in UTC; ``SOURCE_DATE_EPOCH`` (seconds since 1970) instead when it is set, for reproducible builds.

    Raises:
        OptionError: When ``SOURCE_DATE_EPOCH`` is not a whole number of seconds in range.
    """
    ...
    try:
        ...
        msg = f"SOURCE_DATE_EPOCH must be a whole number of seconds since 1970, got {epoch!r}"
...
def write_program(
    plan: JobPlan, *, now: Callable[[], datetime] | None = None, provenance: Provenance | None = None
) -> str:
    """Lay out the whole program for ``plan`` and return its text.

    ``provenance`` adds the lines naming what the program was made from;
    without it the header identifies only the tcnc version.
    ...
    writer.header(extras, provenance)
    current_a = 0.0

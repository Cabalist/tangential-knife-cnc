...
from tcnc.provenance import ComponentVersion, Provenance
...
def test_identity_lines_ignore_the_comment_setting_and_the_timestamp_is_optional() -> None:
    plan = plan_toolpaths([square()], QUIET)

    def comments(text: str) -> list[str]:
        return [line for line in text.splitlines() if line.startswith(";")]

    # A plan built from toolpaths has no files to name: only the version identifies it.
    assert comments(write_program(plan, now=lambda: FIXED_NOW)) == [f"; Generated by tcnc {__version__}"]
    provenance = Provenance(from_options=True, versions=(ComponentVersion("tcnc", "9.9.9"),))
    assert comments(write_program(plan, now=lambda: FIXED_NOW, provenance=provenance)) == [
        f"; Generated by tcnc {__version__}",
        "; job-file: none (command-line options)",
        "; versions: tcnc 9.9.9",
    ]
    stamped = write_program(plan_toolpaths([square()], KnifeOptions()), now=lambda: FIXED_NOW)
    assert "; Created 2026-09-12T12:00:00+00:00" in stamped
    unstamped = write_program(plan_toolpaths([square()], KnifeOptions(timestamp=False)), now=lambda: FIXED_NOW)
    assert unstamped == stamped.replace("; Created 2026-09-12T12:00:00+00:00\n", "")
